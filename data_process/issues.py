...
class Issues:
    ...
    def get_issues(self,page_num):
        ...
        try:
            for number in range(page_num,10000):
                url = "https://api.github.com/repos/%s/issues?state=all&per_page=100&page=%d&access_token=adaccd3708619221656a9e13fc77bd8e5270c70a"%(self.repo_fullname,number)
                print number
                ...
                if issue_content=="[]":
                    ...
                    for issue in issues:
                        ...
                        value = (str(self.repo_id),str(url),str(labels_url),str(comments_url),str(events_url),str(html_url),str(issue_id),str(number),str(title),str(user),str(labels),str(state),str(locked),str(assignee_id),str(milestone),str(comments), \
                                 str(created_at),str(updated_at),str(closed_at),str(pull_request),str(body))
                        try:
                            self.cur.execute("insert into issues (repo_id,url,labels_url,comments_url,events_url,html_url,issue_id,number,title,user,labels,state,locked,assignee_id,milestone,comments, \
                                 created_at,updated_at,closed_at,pull_request,body) values (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",value)
                            self.conn.commit()
    ...
    def get_issue_comments(self,page_num):
        ...
        try:
            for number in range(page_num, 10000):
                url = "https://api.github.com/repos/%s/issues/comments?per_page=100&page=%d&access_token=adaccd3708619221656a9e13fc77bd8e5270c70a" % (self.repo_fullname, number)
                print number
                request_content = urllib2.Request(url)
                issue_content = urllib2.urlopen(request_content).read()
                if issue_content == "[]":
